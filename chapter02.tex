...
\begin{itemize}
	...
	\item Typist: Ari Gam.
	\item Description: Implement a small program that contains an interface called \textit{Lookup}. This interface has one method called find. It returns \textit{Object} and receives \textit{String}. a class called \textit{SimpleLookup} that implements the \textit{Lookup}. It has two private members: \textit{Names} that is an array of \textit{String}s, and Values that is an array of \textit{Object}s. The implemented method \textit{find} iterates over all the elements in \textit{Names} and compares every on e of them with \textit{Name}. If it finds such element it returns the element. In-addition a method called processValues that receives: \textit{String[]} names, and \textit{Lookup} table (the program was presented to the speaker during all the experiment).
\end{itemize}
